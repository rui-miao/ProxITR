...
def FastOptBW(X, y, sample_weight=None, n_gammas=10):
    ...
    if sample_weight is None:
        H = torch.eye(X.size(0), dtype=float32, device=device) - torch.ones_like(K_X_euclidean)/X.size(0) 
    else:
...
#%%
class proxITR:
    def __init__(self, A,X,Z,W,Y, learning_rate=0.1, n_epoch=10000, batch_size = 200, n_components = 150, opt='LBFGS', verbose=False):
    ...
    def fit_h0_a0_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components=20, cv=5, index='all'):
        """
        RKHS_h0_a0.predict can be used to calculate h0(W,0,X)
        index is rows of data: if ='all', then use all data, otherwise is a 1 dim np.array
        """
        ...
        RKHS_h0_a0 = ApproxRKHSIVCV(gamma_f=gamma_f,n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, cv=cv, n_components=n_components).fit(
    ...
    def fit_h0_a1_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components=20, cv=5, index='all'):
        """
        RKHS_h0_a1.predict can be used to calculate h0(W,1,X)
        index is rows of data: if ='all', then use all data, otherwise is a 1 dim np.array
        """
        ...
        RKHS_h0_a1 = ApproxRKHSIVCV(gamma_f=gamma_f,n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, cv=cv, n_components=n_components).fit(
                      np.concatenate((self.W,self.X),axis=1)[index,:],
                      self.Y[index],
                      np.concatenate((self.Z,self.X),axis=1)[index,:])
    ...
    def predict_h0(self, n_components=20, gamma_f='auto', gamma_h=0.1):
        ...
        return
    def predict_h0_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components, cv):
        """
        Evaluate the performance of fit_h0_a0 and fit_h0_a1 using all data
        """
        self.h0_est = np.ndarray(self.A.shape[0])
        _,_,_,predict_h0_a0 = self.fit_h0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv, index='all')
        _,_,_,predict_h0_a1 = self.fit_h0_a1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv, index='all')
        self.h0_est[(1-self.A).astype(bool)] = predict_h0_a0(np.concatenate((self.W,self.X),axis=1)[(1-self.A).astype(bool),:])
        self.h0_est[self.A.astype(bool)]     = predict_h0_a1(np.concatenate((self.W,self.X),axis=1)[self.A.astype(bool),:])
        return
    ...
    def Delta(self, n_components=20, gamma_f='auto', gamma_h=0.1):
        ...
        return
    def Delta_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components=20, cv=5):
        """
        Calculate Delta_est using all data
        """
        gamma_f_h0_a0, gamma_h_h0_a0, alpha_scale_h0_a0, predict_h0_a0 \
            = self.fit_h0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv)
        gamma_f_h0_a1, gamma_h_h0_a1, alpha_scale_h0_a1, predict_h0_a1 \
            = self.fit_h0_a1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv)
    ...
    def fit_q0_a0_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components=20, cv=5, index='all'):
        """
        RKHS_q0_a0.predict can be used to calculate q(Z,0,X) =>q0(Z,X)
        """
        if _check_all(index): 
        ...
        RKHS_q0_a0 = ApproxRKHSIVCV_q(gamma_f=gamma_f,n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, cv=cv, n_components=n_components).fit(
                      np.concatenate((self.Z, self.X),axis=1), 
                      np.ones_like(index), 
                      np.concatenate((self.X, self.W),axis=1), 
                      index)
    ...
    def fit_q0_a1_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components=20, cv=5, index='all'):
        """
        RKHS_q0_a1.predict can be used to calculate q(Z,1,X) =>q1(Z,X)
        """
        if _check_all(index): 
        ...
        RKHS_q0_a1 = ApproxRKHSIVCV_q(gamma_f=gamma_f,n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, cv=cv, n_components=n_components).fit(
                      np.concatenate((self.Z, self.X),axis=1), 
                      np.ones_like(index), 
                      np.concatenate((self.X, self.W),axis=1), 
                      index)
    ...
    def predict_q0(self, n_components, index):
        ...
        return q0_est[index]
    def predict_q0_cv(self, gamma_f, n_gamma_hs, n_alphas, n_components=20, cv=5):
        """
        self.q0_est : calculate estimate of q0(Z_i, A_i, L_i) based on all data
        """
        self.q0_est = np.ndarray(self.A.shape[0])
        self.fit_q0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv)
        self.fit_q0_a1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv)
    ...
    def FastOptBW(self, X, y, sample_weight=None, n_gammas=10):
        """
        Fast Optimal Bandwidth Selection for RBF Kernel using HSIC (Damodaran 2018, IEEE)
        Input: 
            X: covariates of SVM
    ...
    def fit_d1_X_cv(self, gamma_f, n_gamma_hs, n_alphas=20, n_components=50, cv_n=5, cv_r=5, linearity = 'nonlinear', rhos = [0.005, 0.01, 0.05, 0.1, 0.5], learning_rate=None):
        ...
        if (not 'gamma_f_h0_a0' in self.__dict__) or (not 'gamma_h_h0_a0' in self.__dict__) or (not 'alpha_scale_h0_a0' in self.__dict__) or (not 'predict_h0_a0' in self.__dict__) or (cv_n != self.cv_h0_a0):
            self.fit_h0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv_n)
        if (not 'gamma_f_h0_a1' in self.__dict__) or (not 'gamma_h_h0_a1' in self.__dict__) or (not 'alpha_scale_h0_a1' in self.__dict__) or (not 'predict_h0_a1' in self.__dict__) or (cv_n != self.cv_h0_a1):
            self.fit_h0_a1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv_n)
        ...
        if linearity=='nonlinear':
            ...
            gamma_SVC = self.FastOptBW(X = X, y = Delta_sign, sample_weight=Delta_abs)
    ...
    def fit_d1_XZ_cv(self, gamma_f, n_gamma_hs, n_alphas=20, n_components=50, cv_n=5, cv_r=5, linearity = 'nonlinear', rhos = [0.005, 0.01, 0.05, 0.1, 0.5], learning_rate=None):
        if learning_rate is None:
            learning_rate=self.learning_rate
        if (not 'gamma_f_h0_a0' in self.__dict__) or (not 'gamma_h_h0_a0' in self.__dict__) or (not 'alpha_scale_h0_a0' in self.__dict__) or (not 'predict_h0_a0' in self.__dict__) or (cv_n != self.cv_h0_a0):
            self.fit_h0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv_n)
        ...
        if linearity=='nonlinear':
            ...
            gamma_SVC = self.FastOptBW(X = XZ, y = Delta_sign, sample_weight=Delta_abs)
    ...
    def fit_d2_XW_cv(self, gamma_f, n_gamma_hs, n_alphas=20, n_components=50, cv_n=5, cv_r=5, linearity = 'nonlinear', rhos = [0.005, 0.01, 0.05, 0.1, 0.5], learning_rate=None):
        if learning_rate is None:
            learning_rate=self.learning_rate
        if (not 'q0_est' in self.__dict__) or (self.cv_q0_a0 != cv_n) or (self.cv_q0_a1 != cv_n):
            self.predict_q0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv_n)
        ...
        if linearity == 'nonlinear':
            ...
            gamma_SVC = self.FastOptBW(X = XW, y = ayq0_sign, sample_weight=yq0_abs)
    ...
    def fit_d3_X_cv(self, gamma_f, n_gamma_hs, n_alphas=20, n_components=50, cv_n=5, cv_r=5, linearity = 'nonlinear', rhos = [0.005, 0.01, 0.05, 0.1, 0.5], learning_rate=None):
        if learning_rate is None:
            learning_rate=self.learning_rate
        if (not 'q0_est' in self.__dict__) or (self.cv_q0_a0 != cv_n) or (self.cv_q0_a1 != cv_n):
            self.predict_q0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv_n)
        ...
        if linearity=='nonlinear':
            transX = Scaler()
            X = transX.fit_transform(self.X)
            gamma_SVC = self.FastOptBW(X = X, y = ayq0_sign, sample_weight=yq0_abs)
            featX = Nystroem(kernel='rbf', gamma=gamma_SVC, random_state=1, n_components=self.n_components).fit(X)
    ...
    def C0C1_cv(self, gamma_f, n_gamma_hs, n_alphas=20, n_components=20, cv=5, train_index='all', test_index='all'):
        if _check_all(train_index):
            train_index = range(self.A.shape[0])
        if _check_all(test_index):
            test_index = range(self.A.shape[0])
        ...
        gamma_f_h0_a0, gamma_h_h0_a0, alpha_scale_h0_a0, pred_h0_a0\
            =self.fit_h0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv, index=train_index)
        gamma_f_h0_a1, gamma_h_h0_a1, alpha_scale_h0_a1, pred_h0_a1\
            =self.fit_h0_a1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv, index=train_index)
        gamma_f_q0_a0, gamma_h_q0_a0, alpha_scale_q0_a0, pred_q0_a0\
            =self.fit_q0_a0_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv, index=train_index)
        gamma_f_q0_a1, gamma_h_q0_a1, alpha_scale_q0_a1, pred_q0_a1\
            =self.fit_q0_a1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv, index=train_index)
        h0_a0_est = pred_h0_a0(np.concatenate((self.W,self.X),axis=1)[test_index,:])
        h0_a1_est = pred_h0_a1(np.concatenate((self.W,self.X),axis=1)[test_index,:])
        q0_a0_est = pred_q0_a0(np.concatenate((self.Z,self.X),axis=1)[test_index,:])
        q0_a1_est = pred_q0_a1(np.concatenate((self.Z,self.X),axis=1)[test_index,:])
    ...
    def fit_d_DR_cv(self, gamma_f, n_gamma_hs, n_alphas=20, n_components=50, cv_n=5, cv_r=3, linearity = 'nonlinear', rhos = [0.005, 0.01, 0.05, 0.1, 0.5], learning_rate=None):
        if learning_rate is None:
            learning_rate=self.learning_rate
        C0_all, C1_all, paras = self.C0C1_cv(gamma_f=gamma_f, n_gamma_hs=n_gamma_hs, n_alphas=n_alphas, n_components=n_components, cv=cv_n, 
                                   train_index='all',test_index='all')
        C1_C0_sign_cv = np.sign(C1_all-C0_all)
        if linearity == 'nonlinear':
            transX = Scaler()
            X = transX.fit_transform(self.X)
            gamma_SVC = self.FastOptBW(X = X,
